...
for i in range(1,200000+1):
    ...
    # res.flush()
    start=time.time()
    configFile = './configfile/config'+str(random.randint(1, 10))
    res.write(configFile + ',')
    program_feature = exccmd2string('timeout 120 '+csmith+' --probability-configuration ' + configFile +' -o trainprogram'+str(i)+'.c')
