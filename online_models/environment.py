...
class Environment:
    ...
    def execute_config(self, n):
        ...
        # return
        return prog_crash, prog_intra_diversity, list(np.mean(np.array(feature_vectors),axis=0))
    ...
    def constrainCheck(self):
        def EliminateDumplication_shareEffect(keys, values):
            roulette = dict(zip(keys, values))
            roulette_value = [roulette[k] for k in keys]
            # for i in range(len(roulette_value)):
                # if roulette_value[i] !== 0:



            newvalues = [roulette[k] for k in keys]
            return newvalues

        print("checking Constrain")
        for group in self.base_itm_group.keys():
            ...
            if len(valuesset) != len(values):  # dumplicates elem exist
                ...
                for i in range(len(values)):
                    print("check dumplication for " + group + '[' + str(i) + '] value ' + str(values[i]))
                    print(values)
                    if values[i] == 0:
                        continue
                    step = 0
                    while(values.count(values[i]) != 1):
                        values[i] = int(np.clip(np.random.normal(values[i], 10), 0, 100))  # 高斯随机 exploration=2, 压缩值域0-100
                        print("generate "+group+'['+str(i)+'] value as: '+ str(values[i]))
                        step += 1
                        if step == 8:
                            values[i]=0
                            break

                self.base_itm_group[group] = dict(zip(keys, values))
                print(self.base_itm_group[group])
