    ...
    def execute_config(self, n):
        ...
        for i in range(1, n + 1):
            ...
            if not os.path.exists('./trainprogram' + str(i) + '_0') or not os.path.exists(
                ...
                if check_time:
                    rectime = open('time.txt', 'a')
                    rectime.write("generate checksum1:" + str(end - start) + '\n')
                    rectime.flush()
                    rectime.close()
                if (end - start) >= 60:
                    exccmd('rm trainprogram*')
                ...
                if check_time:
                    rectime = open('time.txt', 'a')
                    rectime.write("generate checksum2:" + str(end - start) + '\n')
                    rectime.flush()
                    rectime.close()
                if (end - start) >= 60:
                    exccmd('rm trainprogram*')
                ...
                if check_time:
                    rectime = open('time.txt', 'a')
                    rectime.write("generate checksum3:" + str(end - start) + '\n')
                    rectime.flush()
                    rectime.close()
                if (end - start) >= 60:
                    exccmd('rm trainprogram*')
                ...
                if check_time:
                    rectime = open('time.txt', 'a')
                    rectime.write("generate checksums:" + str(end - start) + '\n')
                    rectime.flush()
                    rectime.close()
                if (end - start) >= 60:
                    exccmd('rm trainprogram*')
        ...
        if len(feature_vectors)!= 0:
            ...
            actual_fail = bilabels.count(categories_dir['crash'])/len(bilabels)# or wrongcode, both is 1
            ...
            return actual_fail, predict_fail, prog_intra_diversity, list(np.mean(np.array(feature_vectors),axis=0))
        else: # config timeout
